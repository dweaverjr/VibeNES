...
class Ram final : public Component {
  public:
	...
	void debug_print(Address start = 0x0000, std::size_t length = 256) const {
		std::cout << "RAM Dump (starting at $" << std::hex << std::uppercase << start << "):\n";

		for (std::size_t i = 0; i < length && (start + i) < RAM_SIZE; ++i) {
			if (i % 16 == 0) {
				std::cout << std::hex << std::uppercase << (start + i) << ": ";
			}

			std::cout << std::hex << std::uppercase << static_cast<int>(memory_[start + i]) << " ";
